...
const conststartreg = () => { return /^const\s(?=\w+)/ };
const constrequirereg = () => { return /^const\s\w+\s=\srequire\(".+\"\)/ };
const logsDir = "./logs";
...
process.argv.slice(2).map(a => {
    if (/^\-+\w+$/g.test(a)) {
        const b = a.replace(/^\-+/, "");
        const c = Object.keys(processArgs).filter(a => processArgs[a].keys.includes(b))?.[0];
        if (!c) return lastCommand = undefined;
        const d = processArgs[c];
        d.args = [];
        lastCommand = d;
    } else if (lastCommand && lastCommand.allowArgs && (!lastCommand.allowArgs || !lastCommand.argsLength || (lastCommand.args.length < lastCommand.argsLength))) {
        lastCommand.args.push(a);
    };
});

const dirPath = (processArgs.d?.args?.[0] ?? ((process.argv[2]?.replace(/^\-\w+/, "")?.length ?? 0) !== 0 ? process.argv[2] : undefined));
if (!dirPath) throw Error("No dirPath defined");

const skippedDirNames = (processArgs.sd.args ?? skippedDirNamesDefault).join(";").split(";");
const skippedFileNames = (processArgs.sf.args ?? skippedFileNamesDefault).join(";").split(";");
...
function rd(p) {
    ...
    dir.forEach(entry => {
        ...
        if (entry.isDirectory()) {
            ...
            if (!skippedFileNames.includes(entry.name) && /\.js$/.test(entry.name)) jsFilePaths.push(path.resolve(p, entry.name));
        };
    });
};
...
jsFilePaths.forEach(jsFilePath => {
    ...
    for (let i = 3; i < jsFileSplits.length; i++) {
        ...
        if (!conststartreg().test(line)) break;
        if (constrequirereg().test(line)) {
            logs.push({ type: "REPLACE", filePath: jsFilePath, lineIndex: i, line: line });
    ...
    if (!dryRun && fileChanged) fs.writeFileSync(jsFilePath, jsFileSplits.join("\n"));
});

const jsFileChangeLength = (Date.now() - jsFileReadStart);
const jsFullLength = (Date.now() - start);
log(`${dryRun ? "[DRY] " : ""}Found ${!dryRun ? "and replaced " : ""}${matches.length} matches`);
log(`(Searching Directory took ${dirScanLength} ms; Reading/Searching ${!dryRun ? "and Replacing " : ""}Files Took ${jsFileChangeLength} ms; Whole process took ${jsFullLength} ms)`);

if (!fs.existsSync(logsDir)) fs.mkdirSync(logsDir);
let logfilepath = `${logsDir}/log-${new Date(new Date().setMinutes(new Date().getMinutes() - new Date().getTimezoneOffset())).toISOString()}.json`;
